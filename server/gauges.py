...
def show_hurricanes(date):
  cur = g.db.cursor(cursor_factory = psycopg2.extras.RealDictCursor)
  cur.execute("""SELECT stormid, name, to_char(DATE(iso_time), 'YYYY-MM-DD') as dt, latitude as lat, longitude as lon, wind, pres, windpercentile as windp, prespercentile as presp from hurricane  where track_type='main' and stormid in (select distinct stormid from hurricane where iso_time=%(time)s)  ORDER BY stormid,dt;""", {"time":date})
  temp = cur.fetchall()
  print(temp)
