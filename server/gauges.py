...
def show_hurricanes(date):
  cur = g.db.cursor(cursor_factory = psycopg2.extras.RealDictCursor)
  cur.execute("""SELECT stormid, name, to_char(iso_time, 'YYYY-MM-DD HH24:MI') as dt, latitude as lat, longitude as lon, wind, pres, windpercentile, prespercentile, track_type from hurricane where stormid in (select distinct stormid from hurricane where iso_time=%(time)s)""", {"time":date})
  temp = cur.fetchall()
  print(temp)
